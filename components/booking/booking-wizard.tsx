...
export function BookingWizard() {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      {errorMessage && (
        <div className="mb-6 p-4 rounded-xl bg-red-950/60 border border-red-800 text-red-300 text-sm animate-fade-up">
          {errorMessage}
        </div>
      )}
      ...
      {currentStep === 4 && (
        <div className="glass-card rounded-3xl p-8 md:p-10 border border-gold/20 shadow-2xl animate-fade-up space-y-8">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Please check your itinerary carefully before placing your reservation.
            </p>
          </div>
          ...
          <div>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {[
                {
                  id: "cash_to_driver",
                  label: "Pay Chauffeur (Cash / Card)",
                  desc: "Contactless card or cash directly in vehicle",
                  icon: Banknote,
                },
                {
                  id: "pay_online",
                  label: "Pre-Pay Online Link",
                  desc: "Secure payment link sent via email",
                  icon: CreditCard,
                },
                {
                  id: "invoice",
                  label: "Corporate Account Invoice",
                  desc: "Monthly billing for registered accounts",
                  icon: FileText,
                },
              ].map((p) => {
                const Icon = p.icon;
                ...
                return (
                  <div
                    ...
                    className={`cursor-pointer rounded-2xl border p-4 transition-all ${
                      isSelected
                        ? "border-gold bg-gold/15 text-gold shadow-sm font-semibold"
                        : "border-white/5 bg-onyx/40 text-muted-foreground hover:border-white/20"
                    }`}
                  >
                    <Icon className="h-5 w-5 mb-2 text-gold" />
          ...
          <div className="flex justify-between pt-4">
            ...
            <button
              ...
              className="rounded-full btn-gold px-9 py-4 text-sm font-semibold shadow-gold inline-flex items-center gap-2"
            >
              {isSubmitting ? (
                <>
      ...
      {currentStep === 5 && submittedBooking && (
        <div className="glass-card rounded-3xl p-8 md:p-12 border border-gold/40 shadow-2xl text-center animate-fade-up max-w-2xl mx-auto">
          ...
          <div className="mt-6 inline-block rounded-2xl border border-gold/40 bg-gold/10 px-6 py-3">
            <p className="text-xs uppercase tracking-widest text-muted-foreground">Your Booking Reference</p>
          ...
          <p className="mt-6 text-sm text-muted-foreground leading-relaxed max-w-md mx-auto">
            A confirmation receipt has been dispatched to <strong>{submittedBooking.passengerEmail}</strong>.
            Our 24/7 operations team is assigning your chauffeur.
          </p>

          <div className="mt-8 p-6 rounded-2xl bg-onyx/80 border border-white/5 text-left text-xs space-y-2">
            ...
            <div className="flex justify-between pt-1">
              <span className="text-gold font-semibold uppercase">Estimated Total</span>
